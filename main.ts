...
document.addEventListener('keydown', e => {
  ...
  let noteDetail = getNoteDetails(e.key)
...
function getNoteDetails(key: string) {
  return NOTE_DETAILS.find(obj => obj.key === key.toUpperCase())
}
...
function playNotes(n: {
  ...
  NOTE_DETAILS.forEach(n => {
    const keyElement = document.querySelector(`[data-note=${n.note}]`) as HTMLDataElement
